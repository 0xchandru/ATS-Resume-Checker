...
def _load_skills(conn, kb: str):
    ...
    categories = {}
    cat_data = _load_json(os.path.join(kb, "skills/skills_by_category.json"))
    if cat_data:
    ...
    for fname, source in files:
        data = _load_json(os.path.join(kb, fname))
        if not data:
            continue
...
def _load_certifications(conn, kb: str):
    ...
    pat_data = _load_json(os.path.join(kb, "certifications/certifications_regex.json"))
    if pat_data:
...
def _load_onet_occupations(conn, kb: str):
    rows = []
    data = _load_json(os.path.join(kb, "job-titles/onet_occupations.json"))
    if data:
        entries = data if isinstance(data, list) else list(data.values()) if isinstance(data, dict) else []
        for entry in entries:
            if not isinstance(entry, dict):
    ...
    for fname in files:
        ...
        if isinstance(data, dict) and fname.endswith("action_verbs_by_category.json"):
            for cat, verbs in data.items():
                verbs_list = verbs if isinstance(verbs, list) else []
                for v in verbs_list:
...
def _load_companies(conn, kb: str):
    ...
    alias_data = _load_json(os.path.join(kb, "companies/companies_aliases.json"))
    alias_seen = set()
    if alias_data:
        if isinstance(alias_data, dict):
            for alias, canonical in alias_data.items():
                ...
                if an in alias_seen:
                    continue
                alias_seen.add(an)
                alias_rows.append({"alias": alias, "alias_normalized": an, "canonical_name": canonical if isinstance(canonical, str) else str(canonical)})
...
def _load_jd_frequency(conn, kb: str):
    rows = []
    data = _load_json(os.path.join(kb, "job-descriptions/jd_keyword_frequency.json"))
    if not data:
        return
    entries = data if isinstance(data, list) else list(data.values()) if isinstance(data, dict) else []
    for entry in entries:
