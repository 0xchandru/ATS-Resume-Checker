...
import { Sparkles, AlertTriangle, CheckCircle2, XCircle, ChevronDown, ChevronUp, Loader2, ArrowRight, ShieldAlert, BarChart2, Lightbulb, ClipboardList } from "lucide-react";
...
export default function AIVerdict({ evaluation, isLoading, error, onRun }: Props) {
  ...
  const visibleProblems = evaluation
    ? (showAll ? evaluation.problems : evaluation.problems.slice(0, 4))
    : [];
      ...
      {!evaluation && !isLoading && !error && (
        <div className="flex flex-col items-center justify-center py-12 gap-3 text-muted-foreground px-5 text-center">
          ...
          <p className="text-xs opacity-60 max-w-xs">
            Applies your prompt rules — no keyword inflation, no stopwords, brutally honest verdict and fix plan.
          </p>
          <button
            onClick={onRun}
      ...
      {evaluation && !isLoading && (
        <div className="p-5 space-y-5">
            ...
            <div>
              ...
              {evaluation.problems.length > 4 && (
                ...
                >
                  ...
                  {showAll ? "Show less" : `Show ${evaluation.problems.length - 4} more`}
                </button>
              )}
            </div>
          )}
          ...
          {evaluation.missing_requirements.length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {evaluation.missing_requirements.map((r, i) => (
                  <span key={i} className="text-xs px-2.5 py-1 rounded-full bg-amber-500/10 border border-amber-500/25 text-amber-400 font-medium">
                    {r}
                  </span>
                ))}
              </div>
          ...
          <p className="text-xs text-muted-foreground/50 text-center pt-1">
            AI analysis — verify claims against the actual job description
          </p>
        </div>
      )}
    </div>
